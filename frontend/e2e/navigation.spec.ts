...
test.describe("Navigation and Wallet Connection", () => {
  ...
  test("should navigate between pages", async ({ page }) => {
    ...
    await page.getByRole("link", { name: "reui" }).click();
    await expect(page).toHaveURL("/");
  });

  test("should display wallet connection button when not connected", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    const connectButton = page
      .locator("button")
      .filter({ hasText: /connect|Connect|wallet|Wallet/i });
