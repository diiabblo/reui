...
export default function SignInPage() {
  ...
  const handleConnectError = useCallback(
    (error: unknown) => {
      ...
      if (isRecoverableError(error) && retryCount < MAX_RETRY_ATTEMPTS) {
        ...
        setConnectionError(
          `Connection issue. ${getWalletErrorMessage(walletError)} Retrying (${nextRetry}/${MAX_RETRY_ATTEMPTS})...`,
        );
  ...
  const getConnectorButtonText = (connectorName: string): string => {
    if (
      ...
      activeConnector?.name === connectorName
    ) {
      return "Connecting...";
    }
    if (
                  ...
                  .map((connector) => {
                    ...
                    return (
                      <motion.button
                        ...
                        aria-label={
                          isActive
                            ? `${connectorInfo.name} - connecting`
                            : `Connect with ${connectorInfo.name}`
                        }
                      >
                        ...
                        {isActive && (
                          <div
                            className="ml-2 w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"
                            aria-hidden="true"
                          ></div>
                        )}
                      </motion.button>
            ...
            <div className="pt-4 mt-6 border-t border-gray-100">
              <p className="text-xs text-center text-gray-500">
                ...
                <button
                  ...
                  onClick={() => {
                    setConnectionError(
                      "For help, please ensure you have a Web3 wallet installed and try again.",
                    );
                  }}
